...
export async function fetchCountries() {
	...
	if (response.status === 200) {
		...
		return countries.filter((country) => country.borders.length > 0);
	} else throw new Error(response.status);
}

export function pickRandomCountry(noBorderCountries, history) {
	const mainCountry = shuffleArray(noBorderCountries)[0];
	if (!history.includes(mainCountry)) {
		history.push(mainCountry);
		return mainCountry;
	} else {
		return pickRandomCountry(noBorderCountries, history);
	}
}

export function cardPick(mainCountry) {
	let neighbours = [
		...countries.filter((country) =>
			mainCountry.borders.includes(country.code3)
		),
	];
